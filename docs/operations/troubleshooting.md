...
- Read **Log** in UI or `/jobs/<id>/openfoam.log`

## Empty case in OpenFOAM container (AWS)

- `JOBS_VOLUME_NAME` must match across backend, runner, and child container
- See [deploy/aws/README.md](../../deploy/aws/README.md#jobs-volume-openfoam-runs)
...
- Valid `ai/credentials/key.json` and Vertex-enabled GCP project
