"""Internal runner service: starts short-lived solver containers via Docker socket."""
...
app = FastAPI(
    ...
    openapi_tags=[
        {"name": "health", "description": "Docker and volume configuration"},
        {"name": "internal", "description": "Runner API for simulation service only"},
    ],
)
...
JOBS_VOLUME_NAME = os.environ.get("JOBS_VOLUME_NAME", "forge-jobs-data")
...
class RunRequest(BaseModel):
    ...
    """Shell commands run inside the case directory after solver env is sourced (e.g. blockMesh && simpleFoam)."""
...
def run_case(req: RunRequest) -> RunResponse:
    ...
    _logger.info("internal/run start job_id=%s", req.job_id)
    ...
    log_path = JOBS_ROOT / req.job_id / "forge.log"
    log_path.parent.mkdir(parents=True, exist_ok=True)

    # Run inside a short-lived solver container.
    #
    # Important (macOS / Docker Desktop):
    # We cannot bind-mount /jobs/... paths because the Docker daemon needs HOST paths.
    # Instead we mount the named volume that backs /jobs into the solver container.
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Failed to start solver container: {exc}") from exc

    rc = -1
    try:
        # Stream to disk during the run so the backend can tail forge.log and the UI shows live progress.
    ...
    _logger.info("internal/run done job_id=%s returncode=%s", req.job_id, rc)
    return RunResponse(
        job_id=req.job_id,
        returncode=rc,
