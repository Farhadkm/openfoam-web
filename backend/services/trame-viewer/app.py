...
def _list_times(case_dir: Path) -> list[str]:
    ...
    # foamToVTK outputs are usually under <case>/VTK/
    vtk_dir = of_root / "VTK"
...
server = get_server("forge-trame-viewer")
...
def _forge_on_server_bind(wslink_server, **_kwargs):
    aio_app = getattr(wslink_server, "app", None)
    if aio_app is None or getattr(aio_app, "_forge_routes_done", False):
        return
    aio_app._forge_routes_done = True
    ...
    r.add_post("/paraview", _noop_paraview)
    r.add_get("/forge-fullbleed.js", _serve_fullbleed_js)
    r.add_get("/forge-bridge.js", _serve_bridge_js)
...
@ctrl.add("forge_bridge_consume_json")
def forge_bridge_consume_json(**_):
...
@ctrl.add("forge_refresh_lists")
def forge_refresh_lists():
...
def forge_vtk_load():
    ...
    try:
        ...
        # foamToVTK commonly produces UnstructuredGrid (.vtu). Use a mapper
...
def _play_fast_load():
    ...
    Falls back to the full forge_vtk_load() if anything goes wrong.
    """
...
@ctrl.add("forge_toggle_play")
def forge_toggle_play():
...
(function () {
  if (document.getElementById("forge-viz-fullbleed")) return;
  var st = document.createElement("style");
  st.id = "forge-viz-fullbleed";
  st.textContent =
    ...
    ".forge-vtk-root{flex:1 1 auto!important;min-height:0!important;width:100%!important;position:relative!important;}";
...
(function () {
  ...
  function setBridgeJson(obj) {
    var wrap = document.getElementById("forge-bridge-json-in");
    ...
    var id =
      cmd === "prev"
        ...
        : cmd === "next"
          ? "forge-cmd-next"
          : cmd === "load"
            ? "forge-trame-load-btn"
            : cmd === "toggle_play"
              ? "forge-cmd-toggle-play"
              : cmd === "refresh"
                ? "forge-cmd-refresh"
                : null;
  ...
  function readBridgeOut() {
    var wrap = document.getElementById("forge-bridge-json-out");
    var inp = bridgeFieldInput(wrap);
    ...
    try {
      ...
      if (window.parent && window.parent !== window) {
        window.parent.postMessage(
          { type: "forge-trame-state", payload: payload },
          parentTargetOrigin()
        );
      }
    } catch (e) {}
  ...
  window.addEventListener("message", function (ev) {
    try {
      ...
      if (ev.data.type === "forge-trame-set-job") {
        var j = String(ev.data.jobId || "").trim();
        if (!j) return;
        scheduleApply(j, !!ev.data.autoLoad);
        return;
      }
      if (ev.data.type === "forge-trame-patch-state") {
        var p = ev.data.patch || {};
      ...
      if (ev.data.type === "forge-trame-cmd") {
        deferUntilSessionLikelyReady(function () {
...
# Relative paths so scripts load under BFF /viewer/… as well as direct :8090/.
...
with VAppLayout(server) as layout:
    with vuetify3.VMain(classes="d-flex flex-column flex-grow-1 pa-0", style="min-height:0;height:100%;"):
        with html.Div(style="display:none", aria_hidden="true"):
            with html.Div(id="forge-bridge-json-in"):
                vuetify3.VTextField(v_model=("bridge_json", ""), density="compact", hide_details=True)
            vuetify3.VBtn(
                "bridge consume",
                click=ctrl.forge_bridge_consume_json,
                id="forge-bridge-consume-btn",
                style="display:none",
            )
            with html.Div(id="forge-bridge-json-out"):
                vuetify3.VTextField(
            ...
            vuetify3.VBtn(".", click=ctrl.step_prev, id="forge-cmd-prev", style="display:none", aria_hidden="true")
            vuetify3.VBtn(".", click=ctrl.step_next, id="forge-cmd-next", style="display:none", aria_hidden="true")
            vuetify3.VBtn(".", click=ctrl.forge_toggle_play, id="forge-cmd-toggle-play", style="display:none", aria_hidden="true")
            vuetify3.VBtn(".", click=ctrl.forge_refresh_lists, id="forge-cmd-refresh", style="display:none", aria_hidden="true")
            vuetify3.VBtn(".", click=ctrl.forge_vtk_load, id="forge-trame-load-btn", style="display:none", aria_hidden="true")
        with html.Div(classes="forge-vtk-root d-flex flex-column flex-grow-1", style="min-height:0;flex:1;width:100%;"):
